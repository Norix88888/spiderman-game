...
(function () {
  ...
  /* 축방향 이탈 벌점. 아래 boneDist 설명 참고 */
  const AXIAL_PENALTY = 1.5;

  /* 정점 p 에서 본 선분 AB 까지의 스키닝용 거리.
       (최단거리) * (1 + AXIAL_PENALTY * 구간이탈률)
     구간이탈률은 투영 매개변수 t 가 [0,1] 을 벗어난 양이다. t 는 이미 선분
     길이로 정규화돼 있으므로, 짧은 본(neck, 9cm)은 담당 구간을 조금만
     벗어나도 지분이 확 깎이고 긴 본(대퇴, 45cm)은 넓게 퍼진다 — 해부학적으로
     맞는 동작이다.

     순수 최단거리를 쓰면 안 되는 이유:
     몸통은 반지름이 0.18m 나 돼서 가슴 정점에서 chest/spine/neck/head 선분
     까지의 최단거리가 전부 0.18~0.32m 로 뭉개진다. 그래서 목을 70도 돌리면
     가슴까지 끌려왔다(수정 전 실측: 유두선 정점이 neck 23% + head 9%).
     반대로 축방향 성분만 쓰면 축이 직교하는 본 쌍(정강이 vs 발)에서 무너진다
     — 발 선분은 거의 수평이라 정강이 한가운데 정점도 축방향 이탈이 얼마 안
     나온다. 그래서 최단거리를 유지한 채 이탈률로 배수만 먹인다.

     관절 한가운데는 양쪽 본 모두 이탈률 0 이고 최단거리도 같으므로 정확히
     50:50 이 된다 — 한 본에 100% 몰리는 조각인형이 구조적으로 불가능하다.
     나눗셈이 없으므로 길이 0 짜리 본이 들어와도 NaN 이 나지 않는다. */
  function boneDist(px, py, pz, a, b) {
    const ux = b[0] - a[0], uy = b[1] - a[1], uz = b[2] - a[2];
    const wx = px - a[0], wy = py - a[1], wz = pz - a[2];
    const dd = ux * ux + uy * uy + uz * uz;
    let t = dd > 1e-12 ? (wx * ux + wy * uy + wz * uz) / dd : 0;
    let over = 0;
    if (t < 0) { over = -t; t = 0; } else if (t > 1) { over = t - 1; t = 1; }
    const dx = wx - ux * t, dy = wy - uy * t, dz = wz - uz * t;
    return Math.sqrt(dx * dx + dy * dy + dz * dz) * (1 + AXIAL_PENALTY * over);
  }
  ...
  const A_UPPER = [             // 재질 3
    ...
    R(1.360, 0.078, 0.074, 2.1, 0.199, -0.001),
    R(1.400, 0.079, 0.075, 2.2, 0.195, -0.002),  // 삼각근 최대 (여기까지 노출)
    /* 아래 두 링은 반드시 몸통 표면 "안쪽" 이어야 한다.
       y=1.424 에서 몸통 반폭 0.267, y=1.438 에서 0.249 이므로
       바깥끝(c1+r1)이 그보다 작아야 어깨 위로 뿔이 솟지 않는다.
       예전 값(1.435 / c1 0.194 / r1 0.073 → 바깥끝 0.267)은 몸통 0.253 을
       넘었고, 게다가 capHi 원뿔 꼭짓점이 y=1.479 까지 올라가 승모근을
       5.7cm 뚫고 나왔다. */
    R(1.424, 0.062, 0.059, 2.2, 0.181, -0.003),  // 삼각근 속으로 진입
    R(1.438, 0.038, 0.036, 2.2, 0.168, -0.004)   // 완전 매몰 (cap 꼭짓점 y=1.459)
  ];
  const A_THUMB = [             // 재질 5
  ...
  const L_THIGH = [             // 재질 6
    ...
    R(0.895, 0.099, 0.098, 2.2, 0.107, -0.002),  // 대퇴 최대 (여기까지 노출)
    /* 골반 반폭은 y=0.940 에서 0.172, y=0.975 에서 0.163 뿐이다.
       예전 값(0.950/0.990 에서 바깥끝 0.209/0.203)은 골반을 4.5cm 뚫고
       나와 장골 높이에 날개처럼 튀어나온 살덩이를 만들었다. */
    R(0.940, 0.090, 0.089, 2.2, 0.105, -0.003),  // 대전자 — 골반 진입
    R(0.975, 0.054, 0.053, 2.2, 0.100, -0.004)   // 골반 속 매몰 (cap 꼭짓점 y=1.005)
  ];
  ...
  function makeSkin(B, W) {
    const S = {   // 본별 대표 선분 (부모위치 → 자식위치, 말단은 가상 끝점)
      ...
      /* 목/머리 경계는 head 본의 실제 회전축(W.head, y=1.57)에 맞춘다.
         예전엔 둘 다 y=1.600 을 썼는데 그러면 블렌드 중심이 회전축보다
         3cm 위에 생겨 고개를 돌릴 때 턱이 목보다 늦게 따라왔다. */
      neck: [W.neck, W.head],
      head: [W.head, [0, 1.845, 0]]
    };
    ...
    /* 신발: 정강이 본 선분이 발목(y=0.05)까지 내려와 신발 속을 관통하기
       때문에 순수 거리로 재면 신발 윗부분이 90% 이상 정강이에 붙어버린다.
       (발을 까딱이면 신발이 찢어진다.) 높이로 직접 지분을 정해준다. */
    const gShoe = function (x, y) { return 0.90 * (1 - ss(0.050, 0.108, y)); };
    ...
    const CAND = {
      ...
      footL: [{ b: 'footL', g: gShoe }, { b: 'kneeL' }],
      footR: [{ b: 'footR', g: gShoe }, { b: 'kneeR' }]
    };
  ...
  function computeSkinAttrs(B, W, boneIndex) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      for (let k = 0; k < set.length; k++) {
        if (set[k].g) continue;
        const seg = S[set[k].b];
        const d = boneDist(x, y, z, seg[0], seg[1]);
        const w = 1 / (d * d * d + e3);
  ...
  function build(opts) {
    ...
    const src = Array.isArray(opts.materials) ? opts.materials : [];
    const mats = [];
    for (let i = 0; i < 9; i++) {
